...
//     try {
//       // Get all members with their monthly amounts
//       const { data: membersData, count: membersCount } = await supabase
//         .from('members')
//         .select('*', { count: 'exact' });

//       // Get all paid subscriptions with member info
//       const { data: subscriptions } = await supabase
//         .from('subscriptions')
//         .select('*, member:members(monthly_amount)')
//         .eq('paid', true);

//       // Calculate total income based on actual member amounts
//       const totalIncome = subscriptions?.reduce((sum, sub: any) => {
//         const memberAmount = sub.member?.monthly_amount || 1000;
//         return sum + memberAmount;
//       }, 0) || 0;

//       const totalPaidMonths = subscriptions?.length || 0;
...
//               <p className="text-xs opacity-75">MRU</p>
//             </div>
...
//               <p className="text-lg sm:text-2xl font-bold">{summary.totalAssistanceAmount.toFixed(2)} MRU</p>
//             </div>
//             <div className="text-3xl sm:text-5xl opacity-80">💰</div>
//           </div>
//         </div>
...
//               <span className="font-bold text-indigo-900">{summary.monthlyContribution.toFixed(2)} MRU</span>
//             </div>
//             <div className="flex justify-between items-center py-2 sm:py-3 border-b text-sm sm:text-base">
//               <span className="text-gray-700">إجمالي الدخل:</span>
//               <span className="font-bold text-green-600">
//                 {((summary.totalPaidMonths * summary.monthlyContribution) || 0).toFixed(2)} MRU
//               </span>
//             </div>
//             <div className="flex justify-between items-center py-2 sm:py-3 border-b text-sm sm:text-base">
...
//                 {summary.currentBalance.toFixed(2)} MRU
//               </span>
//             </div>
//           </div>
//         </div>
...
//                 {summary.totalAssistanceAmount.toFixed(2)} MRU
//               </span>
//             </div>
//             <div className="flex justify-between items-center py-2 sm:py-3 border-b text-sm sm:text-base">
